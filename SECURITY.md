...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/githost-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[githost-mcp] Security Report`.
...
- Credential or token exposure through git remote URLs, API responses, or error messages
- Command injection via repository paths, branch names, or commit messages
- Path traversal in local git operations
- Scope bypass allowing access to repositories or hosts outside the configured remotes
- Dependency vulnerabilities with a plausible exploitation path in githost-mcp's usage
